...
namespace Celeste.Mod.AidenHelper.Entities
{
	...
	class DashKevin : Solid
    {
		...
		public DashKevin(Vector2 position, float width, float height, Axes axes, bool chillOut = false)
		...
		public DashKevin(EntityData data, Vector2 offset)
		...
		public override void Update()
		{
			...
			if (Input.Dash.Pressed)
            {
                foreach (Player entity in base.Scene.Tracker.GetEntities<Player>())
                {
					...
					if (requiresLineOfSight && !IsPlayerSeen(entity, dir) || !CanActivate(dir))
                    {
						return;
                    }
					Attack(dir);
		...
		private IEnumerator AttackSequence()
		{
			...
			while (true)
			{
				...
				{
					speed = Calc.Approach(speed, 24f, CrushAccel * Engine.DeltaTime * 0.25f);
					if (!slowing)
					{
						slowing = true;
						Alarm.Set(this, 0.5f, delegate
